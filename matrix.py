...
class Matrix:
    ...
    ####################################################################
    def __init__(
        self, J, S, Tz, single=True, kl=0, ku=8, N=100, mesh_type="gauleg_finite"
    ):
        ...
        if single == True:
            ...
            if J == 0:
                if S == 0:
                    for i in range(N):
                        for j in range(N):
                            self.vsingle[i][j] = pot.n2losat(
                                J, J, self.MeshPoints[i], self.MeshPoints[j], J, S, Tz
                            )
                elif S == 1:
                    for i in range(N):
                        for j in range(N):
            ...
            for i in range(N):
                for j in range(N):
                    self.vpp[i][j] = pot.n2losat(
                        J + 1, J + 1, self.MeshPoints[i], self.MeshPoints[j], J, S, Tz
                    )
                    self.vpm[i][j] = pot.n2losat(
                        J + 1, J - 1, self.MeshPoints[i], self.MeshPoints[j], J, S, Tz
                    )
                    self.vmp[i][j] = pot.n2losat(
                        J - 1, J + 1, self.MeshPoints[i], self.MeshPoints[j], J, S, Tz
                    )
                    self.vmm[i][j] = pot.n2losat(
                        J - 1, J - 1, self.MeshPoints[i], self.MeshPoints[j], J, S, Tz
                    )
